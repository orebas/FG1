...
void DLGFG(const ComplexPoly &polarg, std::vector<ComplexPoly> pvec, std::vector<ComplexPoly> qvec, long depth);
void solveDebug(mps_context *local_s, mps_polynomial *local_poly, const std::string &polfilename);
void solveCompare(mps_context *local_s, mps_polynomial *local_poly, const std::string &polfilename);
...
public:
    ...
    slong prec;
    slong degree;
    std::map<std::pair<slong, slong>, ACB> coefficients, qcoefficients; // stores DLG coefficients, if and only if they have been calculated.
    // calcCoff calculates the coefficient of x^i in p_k (where p_0 is initial polynomial), stores it in coefficients, and returns it.
            ...
            else
            {
                if (2 * k > degree)
                {
                    for (slong j = 0; j <= k - 1; j++)
                    {
                        if (j % 2 == 0)
                        {
                            result += calcCoeff(p - 1, j) * calcCoeff(p - 1, 2 * k - j); // TODO use better function to times 2
                        }
                        else
                        {
    ...
    ACB calcQcoeff(slong q, slong k)
    {
        ...
        if (k >= 0 && k <= degree)
        {
            ...
            {
                for (slong i = 0; i <= 2 * k + 1; i++)
                { // TODO this is too many iteration, trim it down
                    slong j = 2 * k + 1 - i;
    ...
    ARB
    minRadius(slong depth)
    {
        ARB r(0, prec);
        r = (calcCoeff(depth, 0) / calcCoeff(depth, 1)).abs().root_ui(std::pow(2, depth));
        return r;
    }
    ...
    {
        ARB r(0, prec);
        r = (calcCoeff(depth, degree - 1) / calcCoeff(depth, degree)).abs().root_ui(std::pow(2, depth));
        return r;
    }
    ...
    RecursiveRadiiEstimator(const ComplexPoly &polargtocopy) : p0(polargtocopy), q0(polargtocopy), prec(polargtocopy.intprec), degree(p0.degree())
    {
...
public:
    void generatePnQnPolynomials()
    { // depth and prec must already be set
        ...
        for (slong i = 1; i < depth; i++)
        {
            ...
            // acb_poly_compose(qnegx.pol, qvec[i - 1].pol, negx.pol, qvec[i - 1].intprec);
            // acb_poly_compose(pnegx.pol, pvec[i - 1].pol, negx.pol, pvec[i - 1].intprec);
            t1 = qvec[i - 1] * pnegx;
            ...
            acb_poly_scalar_mul_2exp_si(t3.pol, t3.pol, -1); // at this point t3 should be divisible by x
    ...
    FGSolver(const ComplexPoly &polargtocopy, long initdepth) : p0(polargtocopy), depth(initdepth), prec(polargtocopy.intprec)
    {
...
std::vector<ARB> ACBVectorComp(std::vector<ACB> &a, std::vector<ACB> &b)
{ // compare b with a as the reference  TODO make this const later
...
public:
    ...
    RootSolver1(const ComplexPoly &intpoly, slong initdepth) : poly(intpoly), depth(initdepth), RRE(poly), FGS(poly, initdepth), rootapprox(depth)
    {
    }

    std::vector<ACB> solve(void)
    {
        std::random_device rd; // TODO make this a static object and only have one generator
        std::mt19937 mt(rd());
        ...
        acb_poly_taylor_shift(shiftedpoly.pol, shiftedpoly.pol, randomcenter.c, shiftedpoly.intprec);
