    ...
    ACB calcQcoeff(slong q, slong k) {
        ...
        if (k >= 0 && k <= degree) {
            if (q == 0) {
                ...
                for (slong i = 0; i <= 2 * k + 1; i++) {  // TODO this is too many iteration, trim it down
                    slong j = 2 * k + 1 - i;
...
std::vector<ARB> ACBVectorComp(std::vector<ACB> &a, std::vector<ACB> &b) {  // compare b with a as the reference  TODO make this const later
    ...
    rmsrel /= ARB(a.size(), prec);
    rmsabs /= ARB(a.size(), prec);
